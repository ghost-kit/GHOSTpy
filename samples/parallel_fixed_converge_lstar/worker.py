...
class LStarConvergeWorker(object):
    ...
    def __call__(self, conv_tol=1e-6, data=None, vector=None, pointlist=None, depth=4):
        ...
        if self.pointlist is not None:
            ...
            for point in self.pointlist:
                ...
                if self.conv_tol > 0:
                    ...
                    lshell.converge_p2(depth=self.depth)
    ...
    def get_data(self):
        return np.array(self.loc), np.array(self.lstar), np.array(self.clstar), np.array(self.clines), np.array(self.kval), np.array(self.bmirr)

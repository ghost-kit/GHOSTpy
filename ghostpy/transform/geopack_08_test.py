"""
Exercises the Python bindings to GEOPACK-2008.  You must compile &
install GEOPACK-2008 before this test will pass.  See
pkgs/geopack-2008 for details.
...
class TestGeopack(unittest.TestCase):
    """
    This class tests the GEOPACK-2008 library wrapped via Python & f2py.
    """
    def setUp(self):
        ...
        geopack_08.recalc_08(self.d.year,
    ...
    def test_GEOGSM(self):
        ...
        output = geopack_08.geogsw_08(self.geo[0], self.geo[1], self.geo[2], 0.0,0.0,0.0,  self.iGEOtoGSM)
        ...
        output = geopack_08.geogsw_08(0.0, 0.0, 0.0, output[3], output[4], output[5], self.iGSMtoGEO)
    ...
    def test_SMGSM(self):
        ...
        output = geopack_08.smgsw_08(0.0, 0.0, 0.0, self.gsm[0], self.gsm[1], self.gsm[2], self.iGSMtoSM)
        ...
        output = geopack_08.smgsw_08(output[0], output[1], output[2], 0.0, 0.0, 0.0, self.iSMtoGSM)
    ...
    def test_GSMGSE(self):
        ...
        output = geopack_08.gswgse_08(self.gsm[0], self.gsm[1], self.gsm[2], 0.,0.,0., self.iGSMtoGSE)
        ...
        output = geopack_08.gswgse_08(0.,0.,0., output[3], output[4], output[5], self.iGSEtoGSM)
    ...
    def test_merkin(self):
        ...
        certain inputs.  Compare those inputs here against
        GEOPACK-2005 and CXFORM.
        """
        d = datetime.datetime(year = 2008, month = 8, day = 1,
                              hour = 0, minute = 0, second = 0)

        geopack_08.recalc_08(d.year, d.timetuple().tm_yday,
                             d.hour, d.minute, d.second,
                             -400.0, 0.0, 0.0)

        gse = (0.25, -2.63, -0.4)

        out = geopack_08.gswgse_08(0.,0.,0., gse[0], gse[1], gse[2], self.iGSEtoGSM)
        ...
        out = geopack_08.gswgse_08(out[0],out[1],out[2], 0.,0.,0., self.iGSMtoGSE)
    ...
    def test_cxform(self):
        ...
        out = geopack_08.gswgse_08(0.,0.,0., gse[0],gse[1],gse[2], self.iGSEtoGSM)
        out = geopack_08.smgsw_08(0.,0.,0., out[0],out[1],out[2], self.iGSMtoSM)

        self.__assertPercentDiff( .2830, out[0] )
        self.__assertPercentDiff( -2.6554, out[1] )
        self.__assertPercentDiff( -0.085491587715317729, out[2] )

        # Check the inverse:
        out = geopack_08.smgsw_08(out[0],out[1],out[2], 0.,0.,0., self.iSMtoGSM)
        out = geopack_08.gswgse_08(out[3],out[4],out[5], 0.,0.,0., self.iGSMtoGSE)
